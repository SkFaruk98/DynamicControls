    ...
    function (Controller) {
        ...
        return Controller.extend("project1.controller.OrderPage", {
            onInit: function () {
                ...
                //forSelectOfListOnClickEditColumns
                this.onSetList();
            },
                        ...
                        new sap.m.Button({
                            ...
                            press: () => {
                                if (saveFunction == "Sort")
                                    ...
                                    this.onOrderSave();
                                    else if (saveFunction == "onClickEditOk")
                                    this.onClickEditOk();
                            }
                        }),
                        new sap.m.Button({
            ...
            //on click on the Edit columns button
            onSetList:function(){                    
                 let oTable=this.byId("orderTable").getColumns();
                 let aList=this.byId("listOf").getAggregation("items");
                 oTable.forEach((oHeading,index)=>{
                    let cVisible= oHeading.getVisible();
                    if(cVisible==true)
                        aList[index].setSelected(true)
                    else
                        aList[index].setSelected(false)
                 })
            },
             onClickEdit:function(){
                      let oList=this.byId("listOf");
                      let title="Edit Columns";
                      let saveFunction="onClickEditOk";
                      this.onSetList();
                      this.onDialog(oList,title,saveFunction);
            ...
            onClickEditOk:function(){
                ...
                aList.forEach((oList,index)=>{
                    let lSelected=oList.getSelected()
                    if(lSelected==true)
                        oTable[index].setVisible(true);
                    else
                       oTable[index].setVisible(false);
                                    ...
                                    factory: (oItmIndx, oItmCtx) => {
                                        ...
                                        return new sap.ui.layout.form.FormElement({
                                            ...
                                            fields: new sap.m.Input({
                                                value: `{saleOrderModel>/itemInput/${iPath}/${itemLabel}}`
                                            }),
                                        })
                                    }
                                }
            ...
            onOrderSave: function () {
                ...
                let aOrder = mPath.getProperty("/orders");
                let aItemInput = mPath.getProperty("/itemInput");
                ...
                aOrder.push({
                    customerId: oOrderInput.customerId,
                    orderId: oOrderInput.orderid,
                    orderDate: oOrderInput.orderDate,
                    numberOfItems: oOrderInput.numberOfItems,
                    status: oOrderInput.status,
                    customerName: oOrderInput.customerName,
                    amount: "",
                    paymentType: oOrderInput.paymentType
                })
